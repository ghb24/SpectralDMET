...
!   g95 -c -g -O0 -pedantic -Wall -Wextra -fbounds-check -ftrace=full minresqlpModule.f90
!   See also: Makefile 
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
contains
  ...
  subroutine MINRESQLP( n, Aprod, b, shift, Msolve, disable, nout,        &
    ...
    !                        the file associated with nout is not open properly,
    ...
    intrinsic :: abs, sqrt, present, floor, log10
    ...
    do while (istop_ <= flag0) 
       ...
       if ( .not. precon_ ) then
          ...
          call Msolve( n, r2, y )
          betan = ddot(n, r2, 1, y, 1)           ! betan = betak+1^2
          if (betan > zero) then
             betan = sqrt(betan)
       ...
       if (abs(gama) > eps  .and.  xnorm_tmp < maxxnorm_) then
          ...
          vec2(1)   = xnorm_tmp
          vec2(2)   = u
          if (dnrm2(2, vec2, 1) > maxxnorm_) then
             u      = zero
             istop_ = 12    
          end if
       else     
